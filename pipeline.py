...
def apply_metric_basis(quarters: list[dict], report: dict | None = None) -> list[dict]:
    ...
    ⚠️ 왜 조정 EPS 를 1순위로 올리지 않았는가 (백테스트가 막았습니다)
        자사주를 크게 줄인 4종목에서 두 기준의 큰 방향 일치율은 **85.7%**
        [70.6~93.7] 였습니다. 자사주가 없는 표본에서는 96% 였으니,
        **자사주가 있을 때 더 갈립니다.**

        ⚠️ 처음에 79.5% 라고 적었다가 85.7% 로 정정했습니다. 재검증에서 EBAY 가
           중단사업으로 과거 실적을 세 번 재작성해 5개 분기의 영업이익과 EPS 가
           서로 다른 기준으로 섞여 있다는 것이 드러났습니다. 불일치의 일부는
           EPS 문제가 아니라 제 데이터의 오염이었습니다.
