...
function 못찾음판(말) {
  ...
  return `<div class="card"><div class="t">${esc(코드)} — 자료가 없습니다</div>
    <div class="d">아직 이 종목을 수집한 적이 없습니다.
    아래를 누르면 <b>종목 코드가 미리 채워진 요청장</b>이 열립니다.
    초록 단추(Create)만 누르면 로봇이 바로 수집을 시작합니다.<br><br>
    <a class="back" id="요청단추" data-sym="${esc(코드)}"
       href="${요청주소(코드)}" target="_blank" rel="noopener">▶ ${esc(코드)} 수집 요청</a>
    <div class="n" id="기다림"></div><br>
    <b>기다리는 동안</b>: 이 화면이 20초마다 스스로 확인합니다. 자료가
    도착하면 <b>새로고침 없이</b> 그 종목 화면으로 넘어갑니다
    (보통 2~5분, 처음 보는 종목이면 좀 더 걸립니다).<br><br>
