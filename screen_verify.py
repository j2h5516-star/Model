...
def 화면과_데이터를_맞댄다() -> list[dict]:
    ...
    **값을 고치지 않습니다** — 다른 곳을 찾아 적을 뿐입니다.
    ...
    적기("영업이익률 이상", False,
        ...
        + " — 남은 것은 값을 안 고칩니다. 183차-AB 부터 **XBRL 영업이익이 "
        "직접 다른 말을 하면** 버립니다(그 자가 없거나, 있어도 이웃까지 "
        "함께 오염돼 이웃 검사가 눈이 먼 자리는 남습니다 — 실물 KO 2020년은 "
        "네 분기가 모두 어긋나 서로를 가려 주지 못합니다)",
        확인못함=(마진["이상"] > 0))
            ...
            if m - 중앙 >= 20 and (abs(중앙) < 1 or m / 중앙 >= 2.0):
                ...
                if len(예) < 3:
                    예.append(f"{t} {rows[i].get('period_label')} "
                              f"{m:.0f}%↔이웃 {중앙:.0f}%")
    # 183차-AB — **정제가 이미 버린 칸**도 함께 보여 줍니다. 남은 수만
    # 보면 주인은 "원래 적었다"와 "버려서 적다"를 구별할 수 없습니다.
    버림 = sum(1 for n in (ds.get("notes") or []) if "183차-AB" in str(n))
    return {"검사": 검사, "이상": 이상, "종목": len(종목), "예": 예, "버림": 버림}
