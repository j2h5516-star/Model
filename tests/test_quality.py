...
def test_주단위_회계달력도_같은_분기로_묶인다():
    """(183차-K) 13주 분기 회사는 분기끝이 해마다 며칠씩 밀립니다.

    같은 회계분기가 **어떤 해에는 9월 30일, 다른 해에는 10월 1일**에
    끝납니다. 달(month)로만 번호를 매기면 같은 분기가 해마다 다른 번호를
    받아 계절성·이상값의 '같은 분기끼리 묶기'가 어긋납니다.

    전수 실측(2026-09-13 수집물): 이웃한 두 분기의 번호가 한 칸씩 돌지
    **않는** 쌍이 14,811쌍 중 **313쌍(55종목) → 44쌍(5종목)**.
    (남는 44쌍은 COST·AZO 처럼 12·12·12·16주로 나누는 회사였습니다.
     이것은 183차-X 에서 `회계분기_순번` 으로 따로 풀었습니다 —
     아래 `test_열두주달력_회사도_네_분기로_갈린다` 를 보세요.)
    """
    # ① TMO 실물 — 2016년 3분기는 10월 1일, 2017년 3분기는 9월 30일에 끝남
    a = dq.fiscal_quarter_of({"filing_date": "2016-10-01"})
    b = dq.fiscal_quarter_of({"filing_date": "2017-09-30"})
    assert a[1] == b[1] == 3, (
        f"같은 3분기가 다른 번호를 받았습니다: 2016-10-01 → {a}, 2017-09-30 → {b}")
    assert a == (2016, 3), f"연도까지 가장 가까운 분기끝을 따라야 합니다: {a}"

    # ② 그래도 **서로 다른 분기는 달라야** 합니다 (묶임이 뭉개지면 안 됨)
    같은해 = [dq.fiscal_quarter_of({"filing_date": d})
              for d in ("2016-04-02", "2016-07-02", "2016-10-01", "2016-12-31")]
    assert len(set(같은해)) == 4, f"네 분기가 한 묶음이 됐습니다: {같은해}"

    # ③ KLIC 실물 — 1월 2일에 끝난 분기는 **직전 해 4분기**입니다
    assert dq.fiscal_quarter_of({"filing_date": "2021-01-02"}) == (2020, 4)

    # ④ 날짜가 아니라 달까지만 있으면 달로 정합니다 (없는 값을 만들지 않음)
    assert dq.fiscal_quarter_of({"filing_date": "2021-06"}) == (2021, 2)
...
# ---------------------------------------------------------------------------
# 183차-X — 분기 번호를 **달력이 아니라 그 종목 자기 순서**로 매긴다
# ---------------------------------------------------------------------------
def _분기들(*날들):
    """분기끝 날짜만 있는 행들을 만듭니다 (시험 도우미)."""
    return [{"filing_date": d} for d in 날들]


def test_열두주달력_회사도_네_분기로_갈린다():
    """(183차-X) 12·12·12·16주 달력은 달력 분기끝으로는 안 갈립니다.

    코스트코(COST) 실물 — 달력에 붙이면 2월과 5월이 **같은 칸**에 들어갑니다:

        2025-02-16 → (2025, 1)
        2025-05-11 → (2025, 1)   ← 뭉침
        2025-08-31 → (2025, 3)
        2025-11-23 → (2025, 4)

    계절성 판정과 이상값의 '작년 같은 분기' 짝짓기가 여기에 걸려 있어
    화면 오탐보다 무겁습니다 — 자료를 지울지 말지 정하는 자리입니다.
    """
    날들 = ("2025-02-16", "2025-05-11", "2025-08-31", "2025-11-23",
            "2026-02-15", "2026-05-10")
    rows = _분기들(*날들)

    # 옛 방법(달력)은 실제로 뭉칩니다 — 이 시험이 무엇을 고쳤는지 못박습니다
    달력 = [dq.fiscal_quarter_of(r)[1] for r in rows]
    assert len(set(달력[:4])) < 4, f"달력이 이미 네 칸으로 갈렸습니다: {달력}"

    번호 = dq.회계분기_순번(rows)
    첫해 = [n[1] for n in 번호[:4]]
    assert len(set(첫해)) == 4, f"네 분기가 서로 다른 번호를 못 받았습니다: {번호}"
    # 이듬해 같은 분기는 **같은 번호**여야 묶입니다
    assert 번호[4][1] == 번호[0][1], f"2월 분기가 해마다 다른 번호입니다: {번호}"
    assert 번호[5][1] == 번호[1][1], f"5월 분기가 해마다 다른 번호입니다: {번호}"


def test_달력이_맞는_회사는_달력_번호를_그대로_받는다():
    """사람이 읽기 쉬우라고 기준점을 달력에 맞춰 둡니다 (NTRS 실물)."""
    rows = _분기들("2025-03-31", "2025-06-30", "2025-09-30", "2025-12-31")
    assert dq.회계분기_순번(rows) == [(2025, 1), (2025, 2), (2025, 3), (2025, 4)]


def test_같은_분기가_두_줄이면_같은_번호를_받는다():
    """(183차-X) 며칠 차이로 같은 분기가 두 줄 있는 일이 흔합니다.

    실물 WAT: 2017-03-31 과 2017-04-01 이 둘 다 있습니다(63행 중 20쌍).
    무조건 한 칸씩 밀면 그 뒤가 통째로 어긋납니다 — WAT 21칸·NBIX 23칸.
    """
    rows = _분기들("2017-03-31", "2017-04-01", "2017-06-30", "2017-07-01",
                   "2017-09-30", "2017-12-31")
    번호 = [n[1] for n in dq.회계분기_순번(rows)]
    assert 번호 == [1, 1, 2, 2, 3, 4], f"두 줄짜리 분기가 밀렸습니다: {번호}"


def test_군더더기_줄을_징검다리로_밟지_않는다():
    """(183차-X) 기준 날짜는 **칸이 바뀔 때만** 옮깁니다.

    실물 NBIX: 2021-11-01(Q3) · 2021-11-22(군더더기) · 2022-01-06(Q4).
    군더더기를 밟으면 21일 + 45일 이라 둘 다 0칸이 되어 Q4 가 Q3 자리에
    앉습니다. 11-01 에서 곧장 재면 66일 — 한 칸입니다.
    """
    rows = _분기들("2021-08-03", "2021-11-01", "2021-11-22", "2022-01-06")
    번호 = [n[1] for n in dq.회계분기_순번(rows)]
    assert 번호[3] != 번호[1], (
        f"군더더기 줄을 밟아 한 분기를 잃었습니다: {번호}")
    assert 번호 == [2, 3, 3, 4], f"순번이 어긋났습니다: {번호}"


def test_분기가_빠지면_번호도_건너뛴다():
    """수집이 못 한 분기가 있으면 그만큼 번호가 건너뛰어야 짝이 맞습니다."""
    rows = _분기들("2025-03-31", "2025-06-30", "2026-03-31")
    번호 = [n[1] for n in dq.회계분기_순번(rows)]
    assert 번호 == [1, 2, 1], f"빠진 분기를 건너뛰지 않았습니다: {번호}"


def test_날짜를_못_읽으면_없음으로_둔다():
    """헌법 1조 — 없는 값은 만들지 않습니다."""
    rows = [{"filing_date": "2025-03-31"}, {"period_label": "아무 말"},
            {"filing_date": "2025-09-30"}]
    번호 = dq.회계분기_순번(rows)
    assert 번호[1] is None, f"없는 날짜에 번호를 지어냈습니다: {번호}"
    assert 번호[0] is not None and 번호[2] is not None, 번호
    assert len(번호) == len(rows), "입력과 길이가 달라졌습니다"


def test_순서가_뒤섞여_들어와도_제자리에_돌려준다():
    """호출하는 쪽이 값 목록과 **번호로 맞춰** 쓰므로 순서가 어긋나면 안 됩니다."""
    rows = _분기들("2025-09-30", "2025-03-31", "2025-12-31", "2025-06-30")
    번호 = [n[1] for n in dq.회계분기_순번(rows)]
    assert 번호 == [3, 1, 4, 2], f"제자리에 안 돌아왔습니다: {번호}"
