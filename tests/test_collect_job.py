...
# ---------------------------------------------------------------------------
# 183차-AP — **일곱 번째를 막는다**: 심은 계기가 로그까지 가는지 통째로 본다
# ---------------------------------------------------------------------------
# 183차-AO 까지 "만들고 배선을 잊은" 사고가 **여섯 번** 났습니다. 그때마다
# 그 칸 하나를 고치는 시험을 붙였는데, 다음 사람이 새 칸을 만들면 또
# 똑같이 빠집니다. 칸마다 시험을 쓰는 방식으로는 **일곱 번째를 못 막습니다.**
#
# 그래서 규칙 자체를 시험합니다: 수집기가 `report[...]` 에 심는 칸은
# **전부** ① 로그(per_ticker)에 실리거나 ② 아래 "일부러 안 싣는" 목록에
# 이유와 함께 적혀 있어야 합니다. 둘 다 아니면 빨간 불입니다.
_로그에_일부러_안_싣는_칸 = {
    # 칸 이름: 왜 안 싣는가
    "raw_texts": "원문은 data/measure/raw 에 파일로 따로 저장합니다(로그가 부풉니다)",
    "all_errors": "종목당 최대 20건이라 로그가 부풉니다. first_error 로 갈음합니다",
    "text_source": "한 종목 즉시 수집(collect_one)이 필요할 때만 보여 줍니다",
    "hole_filled": "collect_one 이 읽습니다 (매일 런에는 불필요)",
    "pair_note": "collect_one 이 읽습니다",
    "xbrl_ambiguous": "collect_one 이 읽습니다",
}


def _수집기가_심는_칸() -> set:
    """수집기가 진단 리포트에 담는 칸 **전부**.

    ⚠️ 여기가 이 시험의 급소입니다. 처음에는 `report["칸"] = …` 이라는
    **글자**만 찾았는데, 세는 칸들은 `new_report()` 의 사전에 0 으로
    미리 놓이고 `report["칸"] += 1` 로만 늘어나기 때문에 **일곱 칸이
    통째로 시험 밖에** 있었습니다(filings_found·gate_passed 등).
    그래서 사전을 **실제로 만들어** 그 열쇠를 가져옵니다.
    """
    import re
    import inspect
    import sec_fundamentals as sf

    심음 = set(sf.new_report("TEST"))          # 사전을 진짜로 만들어 본다
    수집기 = inspect.getsource(sf)
    심음 |= set(re.findall(r'report\[\s*"([^"]+)"\s*\]\s*=', 수집기))
    심음 |= set(re.findall(r'report\.setdefault\(\s*"([^"]+)"', 수집기))
    return 심음


def _로그에_실리는_칸() -> set:
    """`collect_job` 이 per_ticker 로 로그에 남기는 칸."""
    import re
    import inspect

    로그 = inspect.getsource(cj)
    실림 = set(re.findall(r'"([^"]+)":\s*(?:bool\(|int\(|str\()?r\.get\(', 로그))
    실림 |= set(re.findall(r'"([^"]+)":\s*r\[', 로그))
    return 실림


def test_수집기가_심은_계기는_로그까지_가거나_이유가_적혀_있다():
    """(183차-AP) 칸 하나씩이 아니라 **규칙**을 지킵니다.

    새 계기를 만들고 `collect_job` 의 per_ticker 목록에 이름을 안 넣으면,
    보고에는 담기지만 **로그에는 한 종목도 안 실립니다.** 183차-AO 에서
    실제로 그랬습니다(채운4분기_행적 · 어떤서식을_내나 둘 다 0종목).

    고치는 법은 둘 중 하나입니다:
      ① per_ticker 목록에 `"칸": r.get("칸")` 을 넣는다
      ② 위 `_로그에_일부러_안_싣는_칸` 에 **이유와 함께** 적는다
    """
    빠짐 = sorted(_수집기가_심는_칸() - _로그에_실리는_칸()
                  - set(_로그에_일부러_안_싣는_칸))
    assert not 빠짐, (
        "수집기가 심는데 로그에도 없고 '일부러 안 싣는' 목록에도 없는 칸: "
        f"{빠짐} — per_ticker 에 넣거나 이유를 적으세요 (183차-AO 의 재발)")


def test_일부러_안_싣는_목록에_죽은_이름이_없다():
    """(183차-AP) 반대 방향도 지킵니다 — 없어진 칸이 목록에 남아 있으면
    다음 사람이 "이건 일부러 뺀 것"이라 믿고 지나갑니다.

    두 번째로, **이미 로그에 실리는 칸**이 목록에 남아 있어도 안 됩니다 —
    "안 싣는다"고 적혀 있는데 실제로는 실리면 설명이 거짓말이 됩니다."""
    심음 = _수집기가_심는_칸()
    죽은이름 = sorted(set(_로그에_일부러_안_싣는_칸) - 심음)
    assert not 죽은이름, f"수집기가 더 이상 안 심는 칸이 목록에 남아 있습니다: {죽은이름}"

    거짓설명 = sorted(set(_로그에_일부러_안_싣는_칸) & _로그에_실리는_칸())
    assert not 거짓설명, (
        f"'안 싣는다'고 적혀 있는데 실제로는 로그에 실리는 칸: {거짓설명} "
        "— 목록에서 지우세요")
