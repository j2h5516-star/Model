...
def test_화면_코드가_캐시에_굳지_않게_불린다():
    """(150차-AW) `<script src="app.js">` 로 두었더니, 화면을 고쳐 올려도
    휴대폰이 **예전에 받아 둔 app.js 를 계속 써서** 옛 화면이 그대로
    남았습니다. 주인이 08-25 에 옛 안내문을 보고 알려 주었습니다.

    이 고장은 **아무 오류도 내지 않습니다** — 화면은 멀쩡해 보이는데
    내용만 옛것입니다. 그래서 시험이 대신 봅니다.
    """
    with open(os.path.join(ROOT, "docs", "index.html"), encoding="utf-8") as f:
        html = f.read()
    assert 'src="app.js"' not in html, (
        "index.html 이 app.js 를 그대로 부릅니다 — 휴대폰 캐시에 굳어 "
        "화면을 고쳐도 옛것이 남습니다")
    assert 'app.js?v=' in html, "app.js 주소에 시각(캐시 깨는 값)이 없습니다"
