...
def test_다수와_어긋난_분기이름은_겹치지_않아도_버린다():
    """(183차-Y) 겹치지 않는 **틀린 이름**은 그대로 화면에 남아 있었습니다.

    실물 ECL: 2023-03-31 에 '23 Q2', 2023-06-30 에 '23 Q3' — 보도자료가
    함께 말하는 **다음 분기 전망**을 파서가 물었습니다. 어느 이름과도
    겹치지 않아 175차 규칙에 안 걸립니다.

    전수 실측(2026-09-17 수집물): 이렇게 남아 있던 것이 203칸·83종목.
    어긋난 방향은 다음 분기 116칸 · 직전 분기 62칸 · 두 칸 건너뜀 25칸.

    헌법 1조 — 고치지 않고 **버립니다**. 날짜형은 언제나 사실입니다.
    """
    notes: list[str] = []
    out = dataset._clean_quarters({
        # ⚠️ 이름이 **하나도 겹치지 않게** 짭니다. 겹치면 175차 규칙이
        #    먼저 잡아 이 시험이 새 규칙을 안 보게 됩니다(실제로 그랬습니다).
        "바": [_이름표행("2023-03-31", "23 Q2"),      # ← 다음 분기 이름
              _이름표행("2023-06-30", "24 Q3"),      # ← 다음 분기 + 다른 해
              _이름표행("2023-09-30", "23 Q3"),
              _이름표행("2023-12-31", "23 Q4"),
              _이름표행("2024-03-31", "24 Q1")],
    }, notes)
    이름 = [r["period_label"] for r in out["바"]]
    assert len(set(이름)) == len(이름), f"시험 자료에 겹친 이름이 있습니다: {이름}"
    assert 이름[:2] == ["23/03", "23/06"], f"틀린 이름이 남았습니다: {이름}"
    assert 이름[2:] == ["23 Q3", "23 Q4", "24 Q1"], f"맞는 이름을 건드렸습니다: {이름}"
    assert not [n for n in notes if "겹쳐" in n], (
        f"175차 규칙이 먼저 잡았습니다 — 이 시험은 새 규칙을 못 봅니다: {notes}")
    # 값은 한 칸도 안 바뀝니다
    assert all(r.get("revenue") for r in out["바"]), "값을 건드렸습니다"


def test_겹치지_않아도_어긋난_이름_하나면_버린다():
    """겹침이 없는 순수한 경우 — 175차 규칙으로는 못 잡던 자리입니다."""
    notes: list[str] = []
    out = dataset._clean_quarters({
        "사": [_이름표행("2023-03-31", "23 Q1"), _이름표행("2023-06-30", "23 Q2"),
              _이름표행("2023-09-30", "24 Q2"),      # ← 홀로 어긋남, 겹치지 않음
              _이름표행("2023-12-31", "23 Q4"),
              _이름표행("2024-03-31", "24 Q1")],
    }, notes)
    이름 = [r["period_label"] for r in out["사"]]
    assert len(set(이름)) == len(이름), f"시험 자료에 겹친 이름이 있습니다: {이름}"
    assert 이름[2] == "23/09", f"홀로 어긋난 이름을 못 잡았습니다: {이름}"
    assert 이름 == ["23 Q1", "23 Q2", "23/09", "23 Q4", "24 Q1"], \
        f"맞는 이름을 건드렸습니다: {이름}"
    assert any("회계 달력과 어긋난 분기 이름" in n for n in notes), notes


def test_파서가_문_문장을_버림_기록에_적는다():
    """(183차-Z) 배선 시험 — 만들고 안 실으면 다음 세션이 못 고칩니다.

    원문은 잣대값을 하나도 못 읽었을 때만 보관하므로, 이름만 틀린 행은
    재료가 없습니다(실측: 203칸 중 5칸만 원문 있음). 그래서 파서가 문
    문장 조각을 행에 붙여 보내고, 버릴 때 기록에 함께 적습니다.
    """
    notes: list[str] = []
    행 = _이름표행("2023-03-31", "24 Q2")      # 겹치지 않는 틀린 이름
    행["period_label_문장"] = "guidance for the second quarter 2023 revenue of $1.0B"
    out = dataset._clean_quarters({
        "자": [행,
              _이름표행("2023-06-30", "23 Q2"),      # ← 맞는 이름(다수)
              _이름표행("2023-09-30", "23 Q3"),
              _이름표행("2023-12-31", "23 Q4"),
              _이름표행("2024-03-31", "24 Q1")],
    }, notes)
    assert out["자"][0]["period_label"] == "23/03", out["자"][0]["period_label"]
    적힌 = [n for n in notes if "파서가 문 문장" in n]
    assert 적힌, f"문 문장을 기록에 안 적었습니다: {notes}"
    assert "second quarter 2023" in 적힌[0], 적힌[0]


def test_문_문장이_없어도_기록은_남는다():
    """옛 수집물에는 이 칸이 없습니다 — 그래도 버림 기록은 나와야 합니다."""
    notes: list[str] = []
    dataset._clean_quarters({
        "차": [_이름표행("2023-03-31", "24 Q2"),   # 겹치지 않는 틀린 이름
              _이름표행("2023-06-30", "23 Q2"),
              _이름표행("2023-09-30", "23 Q3"),
              _이름표행("2023-12-31", "23 Q4"),
              _이름표행("2024-03-31", "24 Q1")],
    }, notes)
    적힌 = [n for n in notes if "회계 달력과 어긋난 분기 이름" in n]
    assert 적힌, f"버림 기록이 아예 없습니다: {notes}"
    assert "파서가 문 문장" not in 적힌[0], "없는 문장을 지어냈습니다"


def test_표본이_적으면_이름을_건드리지_않는다():
    """(183차-Y) 서너 칸으로는 '다수'를 말할 수 없습니다 — 헌법 1조."""
    notes: list[str] = []
    out = dataset._clean_quarters({
        "아": [_이름표행("2024-09-30", "24 Q3"), _이름표행("2024-12-31", "25 Q1")],
    }, notes)
    assert [r["period_label"] for r in out["아"]] == ["24 Q3", "25 Q1"], \
        "표본이 둘뿐인데 다수를 말했습니다"
...
def test_XBRL_영업이익이_직접_다른_말을_하면_버린다():
    ...
    실물 SEDG 23 Q4 — 보도자료 +149,600,000 ↔ XBRL -202,651,000.
    (솔라엣지는 그 분기에 크게 적자였습니다.)

    판정 규칙: XBRL 값으로 바꿔 놓으면 **이상이 사라지는가.** 사라지면
    보도자료 쪽이 범인입니다. 그래도 고치지 않고 버립니다 — XBRL 은
    GAAP 이고 보도자료는 논갭이라 한 열에 섞으면 안 됩니다.
    """
    행들 = _열두분기(104_000_000.0 * 0.6, xbrl="다름")   # 매출 자는 못 씀
    행들[4]["op_income_xbrl"] = 104_000_000.0 * 0.10      # 이웃과 같은 10%
    결과 = dataset.build(make_snapshot(eps={"AAA": 행들}))
    kept = 결과["quarters"]["AAA"]
    튄것 = [r for r in kept if r.get("period_label") == "24 Q5"]
    assert 튄것 and 튄것[0]["op_income"] is None, (
        f"XBRL 이 직접 다른 말을 하는데 안 버렸습니다: {튄것}")
    assert 튄것[0]["revenue"] is not None, "매출까지 버리면 안 됩니다"
    assert any("183차-AB" in n for n in 결과["notes"]), "왜 버렸는지 안 적었습니다"
    # **바꿔 넣지는 않습니다** — GAAP 과 논갭을 한 열에 섞지 않습니다
    assert 튄것[0]["op_income"] != 104_000_000.0 * 0.10, \
        "XBRL 값으로 바꿔 넣었습니다 — 버리기만 해야 합니다"


def test_XBRL_영업이익이_같은_말을_하면_안_버린다():
    """(183차-AB) 자가 같은 값을 말하면 **영업이익은 맞는 것**입니다 —
    그때는 매출 쪽이 틀렸습니다(실물 APPF·LITE·SO 3칸)."""
    튄값 = 104_000_000.0 * 0.6
    행들 = _열두분기(튄값, xbrl="다름")
    행들[4]["op_income_xbrl"] = 튄값            # 자가 똑같이 말함
    kept = dataset.build(make_snapshot(eps={"AAA": 행들}))["quarters"]["AAA"]
    튄것 = [r for r in kept if r.get("period_label") == "24 Q5"]
    assert 튄것 and 튄것[0]["op_income"] is not None, (
        "자가 같은 말을 하는데 버렸습니다 — 맞는 값을 잃습니다")


def test_논갭이_조금_높은_것은_버리지_않는다():
    """(183차-AB) 논갭 영업이익은 GAAP 보다 원래 높습니다(주식보상비·
    상각을 되돌리므로). 전수로 재 보니 둘 다 있는 10,102칸 중 차이가
    매출의 5% 를 넘는 것이 2,590칸(25.6%)이었습니다 — XBRL 과 곧장만
    견주면 그 정당한 차이까지 버립니다. 그래서 **이웃 검사를 함께**
    요구합니다."""
    # ⚠️ 이웃에는 **가릴 자를 주지 않습니다.** 자가 있으면 이 함수가
    #    이웃부터 버려서, 문턱을 낮추는 돌연변이가 오히려 이웃을 먼저
    #    먹어 치워 이 칸이 살아남습니다 — 시험이 헛돕니다(실측으로 확인).
    행들 = _열두분기(104_000_000.0 * 0.13, xbrl="없음")  # 마진 13% ↔ 이웃 10%
    행들[4]["op_income_xbrl"] = 104_000_000.0 * 0.05     # GAAP 은 5%
    kept = dataset.build(make_snapshot(eps={"AAA": 행들}))["quarters"]["AAA"]
    튄것 = [r for r in kept if r.get("period_label") == "24 Q5"]
    assert 튄것 and 튄것[0]["op_income"] is not None, (
        "이웃과 어긋나지도 않는 칸을 버렸습니다 — 논갭의 정당한 차이입니다")


def test_두_자가_거의_같으면_문턱_바로_밖이어도_안_버린다():
    """(183차-AB) 이상 문턱 바로 언저리 — 두 자가 **거의 같은 값**을
    말하면 파서가 잘못 읽은 것이 아닙니다.

    보도자료 마진 30.1% · XBRL 마진 29.9% · 이웃 10%. XBRL 로 바꿔도
    이상이 "간신히" 사라지지만(29.9 − 10 = 19.9 < 20), 두 값의 차이는
    매출의 0.2% 뿐입니다 — 같은 값입니다. 이럴 때 버리면 멀쩡한 값을
    잃습니다. 그래서 **벌어짐 문턱**을 함께 봅니다.
    """
    행들 = _열두분기(104_000_000.0 * 0.301, xbrl="다름")
    행들[4]["op_income_xbrl"] = 104_000_000.0 * 0.299
    kept = dataset.build(make_snapshot(eps={"AAA": 행들}))["quarters"]["AAA"]
    튄것 = [r for r in kept if r.get("period_label") == "24 Q5"]
    assert 튄것 and 튄것[0]["op_income"] is not None, (
        "두 자가 거의 같은 값을 말하는데 버렸습니다")
...
# ---------------------------------------------------------------------------
# 183차-AE — **데이터규격.md 가 낡지 않게** 지키는 시험
# ---------------------------------------------------------------------------
# CLAUDE.md 규칙: "코드가 그 문서와 다르면 **코드가 틀린 것**입니다."
# 그런데 실제로는 **문서가 먼저 낡습니다.** 오늘 감사해 보니 네 곳이
# 어긋나 있었습니다:
#
#   · 회계분기 번호를 "기간종료일의 **월**로 정한다"고 적혀 있었는데
#     183차-K 에서 가까운 분기끝으로, 183차-X 에서 종목 자기 순서로
#     두 번 바뀐 뒤였습니다.
#   · 어긋난 분기 이름을 버리는 규칙(183차-Y)이 없었습니다.
#   · 논갭 영업이익을 버리는 세 자격(183차-Q·AB·AC)이 없었습니다.
#   · 시험표에 "150차-AM 에서 추가 예정"이 그대로 남아 있었습니다.
#
# 문서를 읽고 코드를 고치는 사람이 **틀린 문서를 믿게** 됩니다.
# 그래서 문서와 코드가 어긋나면 빨간 불이 되게 묶어 둡니다.
def test_데이터규격이_지금_코드와_맞는다():
    """규격 문서에 적힌 함수·상수가 실제 코드에 있는지 확인합니다."""
    import data_quality as dq
    import os

    뿌리 = os.path.join(os.path.dirname(__file__), "..")
    with open(os.path.join(뿌리, "데이터규격.md"), encoding="utf-8") as f:
        규격 = f.read()

    # ① 회계분기 번호 — 문서가 가리키는 함수가 실제로 있어야 합니다
    assert "data_quality.회계분기_순번" in 규격, \
        "규격이 아직 옛 함수를 가리킵니다"
    assert hasattr(dq, "회계분기_순번"), "규격이 없는 함수를 가리킵니다"
    assert "기간종료일의 **월**로 정한다" not in 규격, \
        "183차-K·X 에서 두 번 바뀐 규칙이 문서에 그대로 남아 있습니다"

    # ② 논갭 영업이익을 버리는 세 자격이 적혀 있어야 합니다
    for 회차 in ("183차-Q", "183차-AB", "183차-AC"):
        assert 회차 in 규격, f"{회차} 의 버리는 자격이 문서에 없습니다"

    # ③ 문서가 말한 문턱이 코드의 값과 같아야 합니다
    assert f"× {dataset._산수_여유:.2f}" in 규격, (
        f"산수 여유가 어긋납니다 — 코드 {dataset._산수_여유}, 문서에 없음")
    assert f"매출의 {int(dataset._XBRL_영업이익_허용오차 * 100)}%" in 규격, (
        "XBRL 영업이익 허용오차가 문서와 어긋납니다")
    assert f"{dataset._마진_초과_PP:.0f}%p" in 규격, \
        "마진 문턱이 문서와 어긋납니다"

    # ④ "추가 예정" 같은 미완성 표시가 남아 있으면 안 됩니다
    assert "추가 예정" not in 규격, "문서에 '추가 예정'이 남아 있습니다"



def test_헌법이_지금_유니버스와_맞는다():
    """(183차-AH) 전략.md 1장은 종목 수를 **"설계가 전제해야 할 현실"**
    로 적어 둡니다. 낡으면 다음 세션이 없는 제약에 맞춰 설계합니다.

    실제로 v2 시절 숫자(79종목 · 5년 · 가이던스 43건)가 그대로 남아
    있었습니다. 지금은 403종목 · 10년 · 가이던스 546건입니다.

    ⚠️ **v2 숫자는 지우지 않습니다** — 그때의 실측입니다. 지금 형편을
    나란히 적고, 종목 수만 코드와 묶어 둡니다(가장 위험한 전제라서).
    """
    import os
    import config as cfg

    뿌리 = os.path.join(os.path.dirname(__file__), "..")
    with open(os.path.join(뿌리, "전략.md"), encoding="utf-8") as f:
        헌법 = f.read()

    assert f"**{len(cfg.TICKERS)}**" in 헌법, (
        f"헌법이 적은 종목 수가 지금({len(cfg.TICKERS)}개)과 어긋납니다")
    # v2 의 옛 실측은 **지우지 않습니다** — 그때의 기록입니다.
    # ⚠️ `"43" in 헌법` 같은 헐거운 검사는 안 됩니다. 그 숫자는 문서
    #    다른 곳("43건 → 546건")에도 나와서, v2 표를 지워도 통과합니다
    #    (돌연변이로 실제로 확인했습니다). 문장 자체를 지킵니다.
    for 옛것 in ("79종목 · 5년 · 발표 있는 분기 1,359",
                "분기 EPS 가이던스는 희소하다 (43건)",
                "| 분기 EPS 가이던스 | **43** |"):
        assert 옛것 in 헌법, f"v2 실측을 지웠습니다: {옛것!r}"
    # 로드맵이 이미 한 일을 "할 일"로 두고 있지 않은가
    assert "H6_결합_H5bxH2b" in 헌법, \
        "결합 가설이 등록됐는데 로드맵이 아직 '할 일'로 적고 있습니다"
