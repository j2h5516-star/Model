...
def test_채운4분기의_행적을_남긴다():
    ...
    실물 LOW(런 #82): q4_채움 은 10개 연말 중 8개를 채웠다고 하고
    못 채운 둘(2015-01-30·2016-01-29)은 수집 시작 이전입니다. 그러니
    2017-02-03(FY2016 4분기)은 **채웠다**는 뜻인데, 수집물에는 그 행이
    없습니다 — 2016-10-28 다음이 곧장 2017-05-05 입니다. 발표일이
    189일 벌어진 구간으로 잡히고, 전수 55건 중 38건이 이런 4분기입니다.

    개발 환경에서는 SEC 가 막혀 재현할 수 없으므로 로봇이 답을 실어
    오게 합니다. `사라짐` 이 0 이면 이 길은 결백합니다.
    """
    report: dict = {}
    계기 = report.setdefault("q4_채움", {}).setdefault("op_income", {})
    sf._fill_missing_q4(
        {"2016-04-29": 1.0, "2016-07-29": 1.0, "2016-10-28": 1.0,
         "2017-05-05": 1.0, "2017-08-04": 1.0, "2017-11-03": 1.0},
        {"2017-02-03": 10.0, "2018-02-02": 10.0},
        계기)
    assert 계기["채움"] == 2, 계기

    # ① 뼈대에 둘 다 있으면 사라진 것이 없습니다
    행적 = sf._채운4분기_행적(report, ["2017-02-03", "2018-02-02"], "2016-09-15")
    assert 행적["op_income"]["사라짐수"] == 0, 행적
    assert 행적["op_income"]["창안"] == 2, 행적

    # ② LOW 증상 — 채웠는데 뼈대에 없습니다. 날짜가 곧 범인의 이름입니다.
    계기["채운날"] = ["2017-02-03", "2018-02-02"]      # 위에서 비웠으므로 다시
    행적 = sf._채운4분기_행적(report, ["2018-02-02"], "2016-09-15")
    assert 행적["op_income"]["사라짐"] == ["2017-02-03"], 행적

    # ③ 수집 시작 이전은 세지 않습니다 — 원래 뼈대에 없는 것이 맞습니다
    계기["채운날"] = ["2015-01-30", "2018-02-02"]
    행적 = sf._채운4분기_행적(report, ["2018-02-02"], "2016-09-15")
    assert 행적["op_income"]["사라짐수"] == 0 and 행적["op_income"]["창안"] == 1, 행적


def test_채운날_목록은_로그에_안_남는다():
    """(183차-AA) 403종목 × 6항목이면 로그가 400KB 부풉니다 —
    우리가 볼 것은 **사라진 날짜**뿐입니다."""
    report: dict = {}
    계기 = report.setdefault("q4_채움", {}).setdefault("op_income", {})
    sf._fill_missing_q4(
        {"2025-03-31": 1.0, "2025-06-30": 1.0, "2025-09-30": 1.0},
        {"2025-12-31": 10.0}, 계기)
    assert 계기.get("채운날") == ["2025-12-31"], 계기
    sf._채운4분기_행적(report, ["2025-12-31"], "2016-09-15")
    assert "채운날" not in 계기, f"날짜 목록이 로그에 남습니다: {계기}"


def test_행적_계기가_수집_보고에_실린다():
    """배선 시험 — 만들고 안 실으면 헛돕니다."""
    with open(os.path.join(os.path.dirname(__file__), "..",
                           "sec_fundamentals.py"), encoding="utf-8") as f:
        코드 = f.read()
    assert 'report["채운4분기_행적"] = _채운4분기_행적(' in 코드, \
        "행적을 만들고 보고에 안 실었습니다"


def test_q4_채움_계기가_실패_이유를_가른다():
    """156차 — 빠진 분기 79건("행 자체가 없음")의 원인을 로봇이 실어
    오게 하는 계기. 채운 것 · 이미 있던 것 · 앞선 세 분기가 모자라
    못 채운 것을 갈래별로 세고, 못 채운 날짜의 예를 남긴다."""
    quarterly = {"2025-03-31": 100.0, "2025-06-30": 110.0, "2025-09-30": 120.0}
    annual = {
        "2025-12-31": 500.0,      # 채울 수 있음 (앞선 3분기 있음)
        "2024-12-31": 400.0,      # 앞선 3분기 없음 → 못 채움
        "2025-09-30": 999.0,      # 이미 분기값이 있음 → 건드리지 않음
    }
    계기 = {}
    filled = sf._fill_missing_q4(quarterly, annual, 계기)
    assert 계기["채움"] == 1, 계기
    assert 계기["이미있음"] == 1, 계기
    assert 계기["앞선세분기부족"] == 1, 계기
    assert 계기["못채운예"] == ["2024-12-31"], 계기
    assert 계기["연간수"] == 3 and 계기["분기수"] == 3, 계기
    # 값 자체는 예전과 똑같아야 한다 (계기는 세기만 한다)
    assert abs(filled["2025-12-31"] - 170.0) < 1e-9, filled
    assert filled["2025-09-30"] == 120.0, "이미 있던 분기값을 덮어썼다"


def test_q4_채움_계기는_연간값이_없는_경우도_남긴다():
    """연간값 자체가 없으면 그 사실을 적는다 — 이 갈래는 파서를 고쳐도
    채울 수 없다(원문에 없는 값). 고칠 수 있는 것과 갈라야 한다."""
    계기 = {}
    sf._fill_missing_q4({"2025-03-31": 1.0}, {}, 계기)
