...
def test_구멍_바로_뒤_발표에_표가_붙는다():
    """(183차-L) H9-clean 이 쓸 "직전 발표에 TTM 이 없었나" 표.

    구멍이 있으면 직전 정점은 구멍 이전 것이라 진짜 정점을 못 본 것일 수
    있습니다 — 그 자리의 "첫 돌파"는 참이라고 말할 수 없습니다.

    실물과 같은 모양: 네 분기를 채워 TTM 이 나온 뒤 한 해를 통째로
    건너뛰고 다시 네 분기가 옵니다.
    """
    행들 = [_행("2022-03-31", 1.0), _행("2022-06-30", 1.0),
           _행("2022-09-30", 1.0), _행("2022-12-31", 1.0),   # 여기서 첫 TTM
           # ── 2023년 통째로 없음 (구멍) ──
           _행("2024-03-31", 2.0), _행("2024-06-30", 2.0),
           _행("2024-09-30", 2.0), _행("2024-12-31", 2.0)]   # 여기서 다시 TTM
    상태 = me.earnings_states(행들, field="gaap_eps")
    표 = [(s["announced"], s["ttm"] is not None, s["직전TTM없음"]) for s in 상태]

    # 첫 TTM 이 나온 발표는 직전 발표에 TTM 이 없었다 → 참
    첫 = [t for t in 표 if t[0] == "2022-12-31"][0]
    assert 첫[1] and 첫[2] is True, f"첫 TTM 자리를 표시 못 했습니다: {표}"

    # 구멍 뒤 다시 TTM 이 나온 발표도 참이어야 한다 (이것이 핵심)
    구멍뒤 = [t for t in 표 if t[0] == "2024-12-31"][0]
    assert 구멍뒤[1] and 구멍뒤[2] is True, f"구멍 뒤를 표시 못 했습니다: {표}"

    # 이어진 자리(직전 발표에 TTM 이 있던 곳)는 거짓이어야 한다
    이어짐 = [t for t in 표 if t[0] == "2024-03-31"][0]
    assert 이어짐[2] is False or 이어짐[1] is False, (
        f"이어진 자리를 구멍으로 표시했습니다: {표}")


def test_사건에_구멍뒤_표가_실려_온다():
    """상태에만 있고 사건에 안 실리면 H9-clean 판정기가 볼 수 없습니다.

    ⚠️ 183차-G·183차-H 에서 **두 번** 같은 사고가 났습니다 — 값을 만들어
    놓고 배선(허용 목록·조립 dict)을 빠뜨려 화면에 0칸이 떴습니다.
    그래서 손으로 만든 dict 가 아니라 **실제로 도는 조립 함수**에서
    시작합니다.
    """
    ds = _mini_ds({"A": _history_rows([1, 1, 1, 1, 2, 3, 5, 8])}, price_days=800)
    events, _ = me.collect_events(ds)
    assert events, "사건이 하나도 없습니다 — 시험 재료가 잘못됨"
    assert all("직전TTM없음" in e for e in events), "사건에 안 실렸습니다"
    assert any(e["직전TTM없음"] is True for e in events), \
        "첫 TTM 자리는 반드시 참이어야 합니다 — 전부 거짓이면 배선이 헛돕니다"
