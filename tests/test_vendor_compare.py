...
def test_flagged_cells_get_a_vendor_verdict():
    ...
    # 183차-AG — 우리 행은 **발표일**로 찾습니다(이름이 아니라).
    표시 = [{"종목": "GS", "칸": "gaap_eps", "라벨": "21 Q4",
           "발표일": "2022-01-18", "값": 59.45},
          {"종목": "AAA", "칸": "gaap_eps", "라벨": "26 Q1",
           "발표일": "2026-04-20", "값": 8.0},
          {"종목": "ZZZ", "칸": "gaap_eps", "라벨": "26 Q1",
           "발표일": "2026-04-20", "값": 3.0}]
...
def test_튄_칸을_이름이_아니라_발표일로_찾는다():
    """(183차-AG) 데이터규격 1장: 이름은 화면 표시용, **계산에 쓰지 않는다**.

    예전에는 `period_label` 로 우리 행을 찾았습니다. 183차-Y 부터 정제가
    어긋난 이름 203개를 날짜형으로 **바꾸므로**, 튄 칸 목록이 런을 넘겨
    저장되는 순간 짝을 놓칩니다.

    ⚠️ 오늘은 하나도 안 깨집니다(전수: GAAP 튄 칸 377개 중 이름으로 못
    찾은 것 0 · 발표일로 못 찾은 것 0) — 튄 칸 목록이 같은 런 안에서
    같은 정제물로 만들어지기 때문입니다. **예방**입니다.
    """
    q = {"AAA": [우리분기("2026-03-31", period_label="바뀐이름/03", gaap_eps=8.0,
                        announced_date="2026-04-20")]}
    v = 보관(AAA=[야후분기("2026-03-31", gaap_eps=2.0)])
    # 이름은 어긋나 있지만 발표일은 맞습니다 → 찾아야 합니다
    표시 = [{"종목": "AAA", "칸": "gaap_eps", "라벨": "26 Q1",
           "발표일": "2026-04-20", "값": 8.0}]
    r = vc.verdict_on_flagged(q, v, 표시)
    assert [x["종목"] for x in r["야후가_다름"]] == ["AAA"], r


def test_발표일이_없으면_아무_행이나_맞히지_않는다():
    """(183차-AG) `None == None` 로 두면 발표일 없는 행이 값만 같으면
    맞아 버립니다 — 시험 자료에서 실제로 그 구멍이 드러났습니다."""
    q = {"AAA": [우리분기("2026-03-31", gaap_eps=8.0)]}      # 발표일 없음
    v = 보관(AAA=[야후분기("2026-03-31", gaap_eps=2.0)])
    표시 = [{"종목": "AAA", "칸": "gaap_eps", "라벨": "26 Q1", "값": 8.0}]
    r = vc.verdict_on_flagged(q, v, 표시)
    assert [x["종목"] for x in r["야후에_없음"]] == ["AAA"], (
        f"발표일이 없는데 짝을 지었습니다: {r}")
