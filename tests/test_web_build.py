...
def test_가설_줄에_신뢰_등급이_붙고_A_B가_먼저_온다():
    """171차 — 화면은 등급 A·B 만 펼친다. 등급은 판정을 만들지 않고 판정
    파일의 구간 수만 읽어 붙인다."""
    verdict = {"가설": {
        "H2_신고점": {"판정": "미채택", "신규(판정)": {
            "신호": {"n": 2449, "rate": 8.8, "ci": [7.8, 10.0]},
            "기준선": {"n": 6991, "rate": 8.8, "ci": [8.1, 9.5]}}},
        "H18_완성시_52주선이격도": {"판정": "판정 불가", "등록일": "2026-08-15",
            "신규(판정)": {"신호": {"n": 0, "rate": None, "ci": [0, 0]},
                        "기준선": {"n": 0, "rate": None, "ci": [0, 0]}},
            "탐색표본(참고)": {"신호": {"n": 713, "rate": 23.6, "ci": [20.6, 26.8]},
                          "기준선": {"n": 4965, "rate": 9.0, "ci": [8.2, 9.8]}}},
        "H9_저평가_첫신기록": {"판정": "채택", "신규(판정)": {
            "신호": {"n": 217, "rate": 12.4, "ci": [8.7, 17.5]},
            "기준선": {"n": 9514, "rate": 8.0, "ci": [7.5, 8.6]}}},
    }}
    rows = wb.hypothesis_rows(verdict)
    assert [r["이름"] for r in rows] == ["H9_저평가_첫신기록", "H18_완성시_52주선이격도", "H2_신고점"]
    assert [r["등급"] for r in rows] == ["A", "B", "C"]
    assert rows[1]["탐색기준선율"] == 9.0 and rows[1]["분리폭"] == 10.8
    assert "완전 분리" in rows[1]["등급이유"]
