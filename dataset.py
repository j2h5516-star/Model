...
def _clean_quarters(eps_map: dict, notes: list[str]) -> dict:
    ...
    for ticker, rows in (eps_map or {}).items():
        ...
        # 183차-AC — 이웃을 안 보는 **산수** 검사를 먼저 돌립니다.
        # 오염된 값을 먼저 걷어내면 아래 이웃 검사의 이웃도 깨끗해집니다.
        _매출총이익을_넘는_영업이익을_버린다(ticker, kept, notes)
        ...
        # 183차-Y — 겹치지 않아도 **그 종목의 다수 규칙과 어긋난** 이름은
        # 버립니다 (아래 함수 설명).
        _다수와_어긋난_분기이름을_날짜형으로(ticker, kept, notes)
        for row in kept:
...
# 다수 규칙과 어긋난 분기 이름은 **고치지 않고 버립니다** (183차-Y)
# ---------------------------------------------------------------------------
# 무엇이 문제였나: 겹치지 않는 틀린 이름은 그대로 화면에 남았습니다.
# 183차-X 로 계기의 오탐을 걷어낸 뒤 전수로 재니 **203칸**이 남았고,
# 세어 보니 어긋난 방향이 한쪽으로 쏠려 있었습니다:
#
#   다음 분기 이름을 달았다  116칸 (57%)
#   직전 분기 이름을 달았다   62칸 (31%)
#   두 분기 건너뜀            25칸 (12%)
#
# 원인은 보도자료 본문입니다. 1분기 발표문에는 **2분기 전망**과
# **작년 1분기 비교**가 나란히 실립니다. 글자로 이름을 뽑는
# `extract_period_label` 이 그중 엉뚱한 문장을 뭅니다. 실물:
#
#   ECL  2023-03-31 → '23 Q2'   (다음 분기 전망을 물었음)
#   EQIX 2022-12-31 → '23 Q1'   (같은 모양)
#   EW   2023-12-31 → '24 Q1'   (같은 모양)
#
# 어떻게 하나: **고치지 않습니다.** 헌법 1조 — 검사에서 탈락한 값은
# 고치지 말고 버립니다. 이름을 기간끝에서 만든 날짜형("23/03")으로
# 바꿉니다. 날짜형은 언제나 사실이고, 틀린 분기 이름은 위험합니다.
# (계산에는 어차피 이름을 안 씁니다 — 기간끝으로 잽니다.)
_이름표_다수_최소표본 = 4


def _다수와_어긋난_분기이름을_날짜형으로(
    ticker: str, rows: list[dict], notes: list[str]
) -> None:
    """그 종목의 다수 규칙과 어긋난 분기 이름을 날짜형으로 바꿉니다."""
    from collections import Counter

    행별차이 = _이름표_분기차이들(rows)
    차이들 = [d for d in 행별차이 if d is not None]
    # 표본이 적으면 '다수'를 말할 수 없습니다 — 건드리지 않습니다
    if len(차이들) < _이름표_다수_최소표본:
        return
    다수 = Counter(차이들).most_common(1)[0][0]

    바꾼것 = []
    문장들 = []
    for row, 차이 in zip(rows, 행별차이):
        if 차이 is None or 차이 == 다수:
            continue
        새이름 = _날짜형_이름표(row)
        if 새이름 is None or 새이름 == row.get("period_label"):
            continue
        바꾼것.append(f"{row.get('period_label')}→{새이름}")
        # 183차-Z — 파서가 **어느 문장에서 이름을 물었는지** 함께 남깁니다.
        # 이것이 있어야 다음 세션이 파서를 고칠 수 있습니다(지금까지는
        # 원문이 없어 203칸 중 5칸만 재료가 있었습니다).
        문장 = row.get("period_label_문장")
        if 문장:
            문장들.append(f"{새이름}: “{문장}”")
        row["period_label"] = 새이름
    ...
    if 바꾼것:
        notes.append(
            f"{ticker}: 이 종목의 회계 달력과 어긋난 분기 이름 {len(바꾼것)}개를 "
            f"기간끝 날짜형으로 바꿨습니다 ({', '.join(바꾼것[:5])}"
            f"{' 외' if len(바꾼것) > 5 else ''}). 보도자료가 함께 말하는 다음 "
            "분기 전망·작년 비교를 파서가 물은 것입니다. 표시만 바뀌고 계산에 "
            "쓰는 값은 그대로입니다"
            + (f" · 파서가 문 문장: {' | '.join(문장들[:3])}" if 문장들 else "")
        )
...
# 183차-AB — 보도자료 영업이익과 XBRL 영업이익의 차이를 **매출로 나눠**
# 잽니다. 영업이익이 0 근처면 비율이 폭발하는데 매출은 안정적입니다.
# 5%p 는 논갭−GAAP 의 흔한 차이(주식보상비·상각)보다 넉넉히 큽니다.
_XBRL_영업이익_허용오차 = 0.05


# 183차-AC — **산수로 확실한 것**: 영업이익은 매출총이익을 넘을 수 없다
# ---------------------------------------------------------------------------
# 183차-AB 가 남긴 사각지대를 메웁니다. 이웃과 견주는 검사는 **이웃까지
# 함께 오염되면 눈이 멉니다.** 실물 KO 2020년은 네 분기가 모두 어긋나
# (5,000M · 6,000M · 5,000M · 7,000M) 서로를 가려 주지 못했습니다.
#
# 이웃이 필요 없는 자가 하나 있습니다 — **산수**입니다.
# 영업이익 = 매출총이익 − 판매관리비이고 판매관리비는 음수가 될 수 없으니,
#
#     영업이익 ≤ 매출총이익
#
# 은 언제나 참입니다. 우리 행에는 `gross_margin_pct` 가 있으므로
# 매출총이익을 바로 구할 수 있습니다.
#
# 전수 실측(2026-09-17 수집물, 세 값이 다 있는 7,240칸):
#
#     영업이익 > 매출총이익 인 칸            259개
#       ├ XBRL 영업이익은 산수를 지킴        155개  ← 보도자료만 틀림, 버린다
#       ├ XBRL 도 함께 위반                  98개  ← 매출총이익률 쪽이 의심,
#       │                                          손대지 않는다(실물 GL 은
#       │                                          보험사라 '매출총이익'이
#       │                                          제 뜻대로 안 씁니다)
#       └ XBRL 이 아예 없음                    6개  ← 가릴 자가 없다, 둔다
#
# ⚠️ 여유를 **10%** 둡니다. 두 가지 때문입니다:
#   ① `gross_margin_pct` 는 비율로 저장돼 되곱하면 끝자리가 흔들립니다.
#   ② **지분법이익·기타영업수익을 영업이익에 넣는 회사**가 있어 산수를
#      몇 % 정당하게 어깁니다(실물 ALB 23 Q1 은 3.3% 초과 — 탈리슨
#      합작사의 지분법이익 때문으로 보입니다).
# 전수로 재 보니 0.5%~10% 사이는 **8칸뿐**입니다(MOS 1.4 · BE 2.5 ·
# DOCU 2.6 · DG 2.7 · URI 3.1 · ALB 3.3 · STRL 4.0 · DG 4.4).
# 그 여덟을 포기하고 147칸을 잡습니다 — 잘못 버리는 것보다 낫습니다.
_산수_여유 = 1.10


def _매출총이익을_넘는_영업이익을_버린다(
    ticker: str, rows: list[dict], notes: list[str]
) -> None:
    """영업이익이 매출총이익을 넘고 **XBRL 은 안 넘는** 행의 영업이익을
    없음으로 만듭니다 (제자리 수정). 이웃을 보지 않습니다."""
    for row in rows:
        rev = row.get("revenue")
        op = row.get("op_income")
        gm = row.get("gross_margin_pct")
        if not (_finite_number(rev) and rev > 0
                and _finite_number(op) and _finite_number(gm)):
            continue
        매출총이익 = rev * gm / 100.0
        if op <= 매출총이익 * _산수_여유:
            continue                      # 산수를 지킵니다
        참영업 = row.get("op_income_xbrl")
        if not _finite_number(참영업):
            continue                      # 가릴 자가 없으면 손대지 않습니다
        if 참영업 > 매출총이익 * _산수_여유:
            continue                      # 자도 함께 위반 — 매출총이익률 쪽 의심
        notes.append(
            f"{ticker} {row.get('period_label', '?')}: 영업이익 {op:,.0f} 이 "
            f"매출총이익 {매출총이익:,.0f}(매출 {rev:,.0f} × {gm:.1f}%)을 "
            f"넘습니다 — 산수로 있을 수 없는 값입니다. XBRL 영업이익 "
            f"{참영업:,.0f} 은 산수를 지키므로 보도자료 값을 없음 처리 "
            "(183차-AC). XBRL 값으로 바꿔 넣지는 않습니다"
        )
        row["op_income"] = None
...
def _이웃과_어긋난_영업이익을_버린다(
    ...
    for i, m in enumerate(마진):
        ...
        # 183차-AB — **XBRL 영업이익이 있으면 그것이 곧 심판입니다.**
        #
        # 183차-P 는 이 14칸을 "바깥 자가 없어 못 가린다"고 적어 두었는데,
        # 세어 보니 **10칸에 `op_income_xbrl` 이 있었습니다.** 안 쓰고
        # 있었을 뿐입니다. 그중 7칸은 부호까지 반대였습니다:
        #
        #   SEDG 23 Q4  보도자료  +149,600,000 ↔ XBRL  -202,651,000
        #   HPE  25 Q2  보도자료 +2,500,000,000 ↔ XBRL -1,109,000,000
        #   KO   20 Q2  보도자료 +6,000,000,000 ↔ XBRL +1,981,000,000
        #
        # 보도자료 쪽이 **무엇인지는 단정하지 않습니다.** 맞춰 보면
        # 여러 갈래가 섞여 있습니다:
        #   · 연간값     ACMR 25 Q4 143.0M ↔ 진짜 분기 23.0M (183차-N 실물)
        #   · 누적값     BKNG 17/08 1,944M ↔ 분기 1,001M (6개월 누적)
        #   · 단위 오독  KO 20 Q2 는 "6.0" 을 읽어 1e9 를 곱한 6,000M
        #               (unit_scale_fixed=1e9). 진짜는 1,981M 입니다.
        # "매출총이익을 집었다"는 갈래도 세어 봤지만 107칸뿐이고 그나마
        # 대부분 보험사(GL)라 큰 건을 설명하지 못했습니다. 그래서
        # **"분기 영업이익이 아니다"까지만** 말하고 버립니다.
        #
        # ⚠️ 왜 이웃 검사를 **함께** 요구하나 — XBRL 과 곧장만 견주면
        #    너무 넓습니다. 전수로 쟀습니다(둘 다 있는 10,102칸):
        #      차이 > 매출의 5%  → 2,590칸(25.6%)
        #      차이 > 매출의 20% →   809칸( 8.0%)
        #      부호가 반대       →   608칸
        #    논갭 영업이익은 GAAP 보다 원래 높습니다(주식보상비·상각을
        #    되돌리므로). 곧장 자르면 그 정당한 차이까지 버립니다.
        #
        # 판정 규칙: XBRL 값으로 바꿔 놓으면 **이상이 사라지는가.**
        # 사라지면 보도자료 쪽이 범인입니다. 그래도 **고치지 않고
        # 버립니다**(헌법 1조) — XBRL 은 GAAP 이고 보도자료는 논갭이라
        # 한 열에 섞으면 분기마다 뜻이 달라집니다(주식보상비에서 이미
        # 겪은 병). 자가 같은 값을 말하면(APPF·LITE·SO) 손대지 않습니다 —
        # 그때는 매출 쪽이 틀린 것입니다.
        참영업 = row.get("op_income_xbrl")
        if _finite_number(참영업):
            xbrl마진 = 참영업 / row["revenue"] * 100.0
            여전히이상 = (xbrl마진 - 중앙 >= _마진_초과_PP
                        and (abs(중앙) < 1
                             or (중앙 > 0 and xbrl마진 / 중앙 >= _마진_초과_배수)))
            벌어짐 = abs(row["op_income"] - 참영업) / row["revenue"]
            if not 여전히이상 and 벌어짐 > _XBRL_영업이익_허용오차:
                notes.append(
                    f"{ticker} {row.get('period_label', '?')}: 영업이익률 "
                    f"{m:.1f}% 가 이웃(중앙 {중앙:.1f}%)과 크게 어긋나는데 "
                    f"XBRL 영업이익은 {참영업:,.0f}(마진 {xbrl마진:.1f}%)입니다 "
                    f"— 보도자료 값 {row['op_income']:,.0f} 은 분기 영업이익이 "
                    "아닌 것으로 보여 없음 처리 (183차-AB). 무엇인지는 "
                    "단정하지 않습니다. XBRL 값으로 바꿔 넣지도 "
                    "않습니다 — GAAP 과 논갭을 한 열에 섞지 않기 위해서입니다"
                )
                row["op_income"] = None
                마진[i] = None
                continue
