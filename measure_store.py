...
EPS_FIELDS = (
    ...
    "gaap_eps",
    # 183차-G — 그 값이 보도자료 조정표의 **분기 열**에서 확인됐는가.
    #
    # ⚠️ 이것이 없으면 183차 전체가 **한 칸도 일하지 않습니다.** 실제로
    #    그랬습니다: 파서·행·정제까지 다 만들고 시험까지 썼는데, 이 허용
    #    목록에 이름을 안 넣어 스냅샷을 만들 때 전부 버려졌습니다
    #    (런 #76 에서 표시 **0칸** 확인). 만들고 배선을 잊은 다섯 번째입니다.
    #
    #    배선 시험은 `_apply_press_to_row` 한 칸만 봤는데, 실제 길은
    #    파서 → 행 → **eps_rows(여기)** → 스냅샷 → 정제로 한 칸 더
    #    길었습니다. 시험은 **값이 쓰이는 끝까지** 따라가야 합니다.
    "adj_eps_분기열",
    "gaap_eps_분기열",
    "adjusted_ebitda_분기열",
    # 183차-Z — 파서가 분기 이름을 **어느 문장에서 물었나** (문장 조각).
    #
    # ⚠️ 183차-AO: 바로 위 주석이 경고한 그 자리에 **또** 걸렸습니다.
    #    파서·행·정제·시험까지 다 만들고 이 목록에 이름을 안 넣어
    #    런 #83 에서 **0건** 찍혔습니다. "만들고 배선을 잊은" 여섯 번째.
    #    시험은 값이 **저장되는 끝까지** 따라가야 합니다.
    "period_label_문장",
    ...
    "gross_margin_pct",
    # 183차-AQ — 바로 위 매출총이익률이 **GAAP 인가 조정인가**.
    #
    # 수집기는 예전부터 이 표시를 행에 달고 있었는데 여기 이름이 없어
    # 스냅샷에서 버려졌습니다. 그래서 스냅샷의 매출총이익률은 GAAP 값과
    # 회사 조정 값이 **구별 없이 섞여** 있었습니다. 조정 매출총이익률은
    # 보통 GAAP 보다 높으므로, 183차-AC 의 산수 규칙(영업이익 ≤
    # 매출총이익)이 행마다 다른 잣대로 재게 됩니다.
    "gm_is_gaap",
    # 183차-AQ — 이 행이 **어디서 왔는지**.
    #   구멍메움 = 짝 없는 8-K 로 빠진 분기를 새로 끼운 행 (150차-Q)
    #   승격     = 뼈대에 없던 8-K 를 그대로 행으로 올린 행
    # 둘 다 수집기가 예전부터 달았거나(구멍메움) 오늘 단(승격) 표시인데,
    # 여기 이름이 없어 화면은 표시를 못 보고 **짐작**으로 갈랐습니다
    # (press_matched 가 비었으면 구멍메움이라고 셌습니다 — 실측 66칸 중
    # 10칸이 둘 다 아니었습니다).
    "구멍메움",
    "승격",
    ...
    "op_income_xbrl",
    "unit_scale_fixed",
    # 183차-CJ — 산수 자(순이자수익+비이자수익)의 0.5~2배 밖이라 **받지 않은**
    # 보도자료 매출 값. 매출 칸은 없음으로 두고, 무엇을 거절했는지는 여기
    # 남깁니다 — 거절이 옳았는지 사람이 되짚을 수 있어야 합니다(감사).
    "은행_보도매출_거절",
    # 183차-CM — 거절할 때의 자(순이자수익+비이자수익). 4분기 자에 연간값이
    # 섞였는지 이웃 분기 자와 견주려고 남깁니다.
    "은행_보도매출_거절_자",
    ...
    "press_matched",
    # 173차 — 6-K 실적 문서(외국 회사)에서 온 행 표시. 격리 규칙의 통행증.
    "fpi_results",
)
