...
_NUMBER_RE = re.compile(
    ...
    # 뒤에 숫자를 요구하므로).
    # 183차-BH — 첫 갈래의 반복이 예전에는 `*`(0회 이상)였습니다. 그래서
    # 콤마 **없는** 네 자리 숫자를 만나면 `\d{1,3}` 이 앞 세 자리만 먹고
    # 반복 0회로 끝나 버렸습니다. 정규식의 `|` 는 **먼저 성공한 갈래**를
    # 쓰므로, 네 자리를 통째로 읽을 두 번째 갈래 `\d+` 까지 가지 못했습니다.
    #   "2023" → 202  ·  "$ 1234 million" → 123 million (**매출의 10분의 1**)
    # `+`(1회 이상)로 바꾸면 첫 갈래는 **콤마가 실제로 있을 때만** 쓰이고,
    # 콤마 없는 숫자는 두 번째 갈래가 통째로 읽습니다.
...
# 183차-BG — 창을 3,000 → **4,500자**로 넓혔습니다.
#
# 왜: GS 2025-01-15 의 연간 표 머리가 값에서 **3,106자** 앞에 있었습니다.
# 106자 차이로 창을 벗어나 아홉 해 중 일곱 해가 안 고쳐지고 있었습니다.
#
# 넓혀도 되는지 전수로 쟀습니다(3,244건 · `parser_quality.py` 의 같은 자):
#   그럴듯 1,380 → **1,411** · 수상 850 → 851
#   갈래 옮김 — 수상→그럴듯 12 ✅ · 수상→없음 3 ✅ · 없음→그럴듯 2 ✅ ·
#              없음→수상 1 ⛔ · 그럴듯→수상 1 ⛔  = **좋아짐 17 · 나빠짐 2**
# 덤으로 183차-AW 가 망가뜨렸던 두 칸도 고쳐졌습니다
# (AZO 365,879,000 → 5,839,501,000 · CL 17,421,000,000 → 4,403,000,000).
_기간머리_창 = 4500          # 값에서 몇 자 뒤까지 표 머리를 찾을까
...
_응답앞_글자 = 1500    # 183차-BC — 300자는 Atom 머리글에서 끊겨 결론을 못 냅니다
...
def _SEC_이름검색(name: str) -> list[list[str]]:
    ...
    if 글 and any((not x[0]) or x[0] == "결과없음" for x in 나온다):
        # 183차-BC — **자가 짧아서 결론을 못 내고 있었습니다.**
        #
        # HOLX 를 네 회차째 "못 찾음"으로 넘기려다 이 계기를 열어 보니
        # SEC 응답이 **300자에서 잘려** 있었습니다. 거기까지는 Atom 머리글
        # (author·id)뿐이고 **결과가 있는지 없는지는 그 뒤**에 나옵니다.
        # 즉 "결과없음"은 파서의 말이고, 원문이 그렇다는 증거는 못 봤습니다.
        # **못 본 것은 "못 봤다"이지 "없다"가 아닙니다**(150차-R 의 교훈).
        #
        # 그래서 ① 창을 넓히고 ② 결과 수를 **직접 세어** 함께 적습니다.
        # entry 가 0 이면 그때는 원문이 "없다"고 말한 것입니다.
        # ⚠️ **줄을 늘리지 않습니다.** 계기를 새 줄로 넣었더니 다른 시험이
        #    그 줄을 회사 번호로 세었습니다(이 목록의 칸 뜻은 [이름, 번호, …]
        #    이고, 읽는 쪽들은 "_응답앞" 한 줄만 건너뜁니다). 한 줄에 담습니다.
        요약 = (f"[entry {글.count('<entry')}개 · company-info "
              f"{글.count('<company-info')}개 · 길이 {len(글)}자] ")
        나온다.append(["_응답앞", 요약 + re.sub(r"\s+", " ", 글[:_응답앞_글자]), ""])
    return 나온다
        ...
        quarters.append(
            {
                ...
                "guidance_text": text,
                # 183차-H — 그 값이 조정표의 **분기 열**에서 확인됐는가.
                #
                # ⚠️ 이 세 줄이 없으면 183차 전체가 한 칸도 일하지 않습니다.
                #    실제로 **두 번** 그랬습니다:
                #      런 #76 — measure_store.EPS_FIELDS 허용 목록에 없어서
                #      런 #77 — **여기**에 없어서 (파서는 남기는데 못 옮김)
                #    이 dict 는 파서 결과를 칸 이름을 하나씩 적어 새로 만드므로,
                #    이름을 안 적으면 **조용히 사라집니다.**
                #
                #    두 번 다 "끝까지 따라가는 시험"을 썼다고 생각했는데,
                #    시험이 **손으로 만든 dict** 로 시작해 이 조립 함수를
                #    건너뛰었습니다. 시험은 **실제로 도는 함수**에서
                #    시작해야 합니다.
...
def _series_for_key(key: str, facts, report: dict | None = None) -> dict[str, float]:
    ...
    #
    # 183차-BE — **세었더니 있었습니다. 이제 버립니다.**
    #
    # 런 #84 전수(183차-AZ): revenue 4칸(ORCL 2019·2020·2021-05-31 등) ·
    # amortization 3 · depreciation_amortization 2 · sbc 1 ·
    # op_income 1(LHX 2026-01-02) — 합계 11칸.
    #
    # 버려도 되는 까닭은 **산수**입니다. 한 분기가 그 해 전체와 같으려면
    # 나머지 세 분기가 0 이어야 합니다. 그러니 이 값은 분기값이 아닙니다 —
    # 짐작이 아니라 증명입니다(183차-AR 이 찾던 "식 밖 자"가 이것입니다).
    #
    # 버리면 그 날짜가 `_fill_missing_q4` 의 손에 들어가 **연간 − 앞 세
    # 분기**로 제대로 채워집니다. 즉 잃는 것이 아니라 **바로잡는 것**입니다.
    #
    # ⚠️ 뼈대가 바뀌는 자리입니다(106차). 그래서 **11칸**이라는 수를 먼저
    #    세고(183차-AS → AZ) 나서 고칩니다. 계기는 그대로 둡니다 — 다음
    #    런에서 이 수가 줄어드는지로 고침이 닿았는지 봅니다.
