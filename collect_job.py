        ...
        "per_ticker": [
            {
                ...
                # 183차-AA 계기 — 채운 4분기가 **뼈대까지 갔는지**.
                # ⚠️ 183차-AO: 이 칸을 여기 안 넣어 런 #83 에서 계기가
                #    한 종목도 안 실렸습니다("만들고 배선을 잊은" 여섯 번째).
                "채운4분기_행적": r.get("채운4분기_행적") or {},
                # 183차-AD 계기 — 한 건도 못 건진 종목이 무슨 서식을 내나.
                "어떤서식을_내나": r.get("어떤서식을_내나") or {},
                # 157차 계기 — 은행 매출 개념 후보가 실제로 있는지.
                # 값은 안 쓰고 세기만 합니다(뼈대는 그대로).
                "은행개념_후보": r.get("은행개념_후보") or {},
                # 183차-CI — 산수로 부분값이 확인되어 **비운** XBRL 매출 칸 수.
                # 다음 런에서 이 합이 183차-CA 계기(127칸)와 비슷해야 합니다.
                "은행_부분값_비움": r.get("은행_부분값_비움", 0),
                # 183차-CJ — 비운 자리에 들어오려던 보도자료 매출 중 자(순이자
                # 수익+비이자수익)의 0.5~2배 밖이라 **받지 않은** 칸 수.
                "은행_보도매출_거절": r.get("은행_보도매출_거절", 0),
                # 183차-CL — XBRL 매출이 **아예 없는** 분기에 자를 붙인 수.
                "은행_자만_붙임": r.get("은행_자만_붙임", 0),
                ...
                # 183차-F 계기 — **짝을 못 찾은 8-K 의 날짜**.
                #
                # 무엇을 판단하려고 재나: 은행 뼈대 문제(160차)의 마지막
                # 물음에 답하기 위해서다. 은행 개념을 뼈대에 넣으면 13종목에
                # 82개 분기가 새로 생기는데(오늘 계기로 확인), **그 빈 행이
                # 보도자료로 채워지지 않으면 오히려 구멍이 된다.**
                # 183차-E 에서 구멍이 첫 돌파의 11.1%를 망친다는 것을
                # 실측했으므로, 채워질 것이 확실할 때만 넣어야 한다.
                #
                # 이 값은 150차-AA 부터 계산되고 있었는데 **로그로 안
                # 나오고 있었다.** 은행의 1월 날짜가 여기 쌓여 있으면
                # "채울 값이 대기 중"이라는 뜻이고, 없으면 넣으면 안 된다.
