...
# --- 계절성 판정 ---
# 분기마다 매출이 오르내리는 '계절 장사'는 전분기 대비(QoQ) 증가율로
# 가속·감속을 판정하면 구조적으로 틀립니다 (백테스트 적중률 0%).
# 그런 종목은 작년 같은 분기와 비교(YoY)해 판정합니다.
SEASONALITY_MIN_QUARTERS = 6      # 이만큼은 있어야 계절성을 따져 봅니다
SEASONALITY_SWING_RATIO = 2.0     # QoQ 흔들림이 YoY의 이 배수를 넘으면 계절성
SEASONALITY_MIN_SWING_PP = 10.0   # 그래도 이 정도(%p)는 흔들려야 계절성으로 봅니다
